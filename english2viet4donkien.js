...
function handleCombine() {
    ...
    let ln = "1"
    const prestr1 = `<div class="out1"><div class="w3 fleft"><p>`
    const prestr2 = `</p></div><div class="w96 fleft">`
    const afstr1 = "</div></div>"
    if (arrEng.length === arrViet.length) {
        ...
        for (let i = 0; i < arrEng.length; i++) {
            ...
            // check if <p> paragraph is the last item in the array
            if (arrEng[i].includes("<p>") && i === arrEng.length - 1) {
                let t1_arrEng = arrEng[i].split(" ")
                if (/^\d+$/.test(t1_arrEng[3])) {
                    ln = t1_arrEng[3];
                    arrEng[i] = arrEng[i].replace(" " + t1_arrEng[3] + " ", " ")
                    arrViet[i] = arrViet[i].replace(" " + t1_arrEng[3] + " ", " ")
                }
                str1 += `${prestr1}${arrEng[i]}${arrViet[i].slice(3)}${afstr1}</p>\n`
            } else if (arrEng[i].includes("<p>") && i < arrEng.length - 1) { // not the last item in the array
                let t1_arrEng = arrEng[i].split(" ")
                if (/^\d+$/.test(t1_arrEng[3])) {
                    ln = t1_arrEng[3];
                    arrEng[i] = arrEng[i].replace(" " + t1_arrEng[3] + " ", " ")
                ...
                if (arrEng[i + 1].includes("<pmid>")) {
                    str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(3)}\n`
                } else if (arrEng[i + 1].includes("<pend>")) {
                    str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(3)}${afstr1}\n`

                }
                else {
                    str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(3)}</p>${afstr1}\n`
                }
            } else {
                let t1_arrEng = arrEng[i].split(" ")
                if (/^\d+$/.test(t1_arrEng[3])) {
                ...
                switch (arrEng[i].slice(0, 4)) {
                    case "<img":
                    case "<vid":
                        str1 += `${prestr1}${prestr2}${arrEng[i]}${afstr1}\n`
                        break;
                    case "<h1>":
                        str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(4)}</h1>\n`
                        break;
                    case "<h2>":
                        str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(4)}</h2>\n`
                        break;
                    case "<h3>":
                        str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(4)}</h3>\n`
                        break;
                    case "<h4>":
                        str1 += `${prestr1}${ln}${prestr2}${arrEng[i]} ${arrViet[i].slice(4)}</h4>${afstr1}\n`
                        break;
                    case "<li>":
                    ...
                    case "<pmi":
                        ...
                        break;
                    case "<pen":
                        str1 += `${arrEng[i].slice(6)} ${arrViet[i].slice(6)}</p>${afstr1}\n`
                        break;
                }
            }
        }
...
img { width: 75%;}
.hide { display: none }
.viet { background-color: yellow;}
.cen { text-align: center; }
.bold { font-weight: bold; }
.out1 { width: 100%; overflow: auto; }
.w3 { width: 3%; }
.w6 { width: 6%; }
.w9 { width: 9%; }
.w12 { width: 12%; }
